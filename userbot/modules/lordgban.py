                ...
                if i.sender == str(guser.id):
                    ...
                    if admin or creator:
                        try:
                            ...
                            await tele.reply(
                                f"**Apollo, Pengguna Gban Telah Bergabung** \n"
...
async def gben(userbot):
    ...
    if user:
        ...
        await dark.edit(f"`Mohon Balas Ke Pesan Pol`")
    try:
        if gmute(user.id) is False:
            return await dark.edit(f"**Kesalahan! Pengguna Ini Sudah Kena Perintah Global Banned Pol.**")
    except BaseException:
        pass
    return await dark.edit(
        f"**࿈ Perintah Apollo:** `{ALIVE_NAME}`\n**✣ Pengguna:** [{user.first_name}](tg://user?id={user.id})\n**✣ Aksi:** `Global Banned`"
    )
...
async def gunben(userbot):
    ...
    me = await userbot.client.get_me()
    await dark.edit(f"`Memulai Membatalkan Perintah Global Banned, Pengguna Ini Akan Dapat Bergabung Ke Grup Anda Pol ヅ`")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    if user:
        if user.id == 1353102497:
            return await dark.edit("**Apollo Pengguna Tidak Bisa Terkena Perintah Ini, Karna Dia Pembuatku ヅ**")
        try:
